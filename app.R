...
ui <- navbarPage(title = "MetaDTA: Diagnostic Test Accuracy Meta-analysis",
                 ...
                 tabPanel("Home", 
                          h1("MetaDTA: Diagnostic Test Accuracy Meta-Analysis v1.45 (17th October 2019)"),
                          br(),
                          h4("If you use this app please cite it as:
                          ...
                          br(),
                          h4("Suzanne Freeman, Clareece Kerby, Amit Patel, Nicola Cooper, Terry Quinn, Alex Sutton"),
                          ...
                          sidebarLayout(
                            ...
                            mainPanel(
                              tabsetPanel(
                                ...
                                tabPanel("ROC Curve",
                                         h5("Note: At least one box under 'Options for ROC Curve tab' must be selected to avoid an error 
                                            message"),
                                         br(),
...
server <- function(input, output) {
  ...
  output$MA_input <- renderUI({
    ...
    div(id=letters[(times %% length(letters)) + 1],
        checkboxGroupInput(inputId = "HSROCcheck", label = h4("Options for ROC Curve tab"),
  ...
  output$extrap <- renderUI({
    if ('2' %in% input$HSROCcheck){checkboxInput(inputId = "extrapp", label = "Extrapolate SROC curve", value = FALSE)}
    else{}
  })
  ...
  output$sroc <- renderPlot({
    ...
    CIdata <- madad(study_level, correction.control = "any")
    # Add the confidence intervals to the dataset
    study_level$Sens_LCI <- CIdata$sens$sens.ci[,1]
    study_level$Sens_UCI <- CIdata$sens$sens.ci[,2]
    study_level$FPR_LCI  <- 1 - CIdata$spec$spec.ci[,2]
    study_level$FPR_UCI  <- 1 - CIdata$spec$spec.ci[,1]
  ...
  output$statTable <- renderTable({
    ...
    Z = data.frame(estimate = c(logit_Sens[1], logit_Spec[1], Sens[1], Spec[1], fpr[1], DOR , LRp , LRn, correlation,
                   ...
                   lci = c(logit_Sens[2], logit_Spec[2], Sens[2], Spec[2], fpr[2], DOR-qnorm(0.975)*se.DOR , LRp-qnorm(0.975)*se.LRp , 
                           LRn-qnorm(0.975)*se.LRn, "NA", "NA", "NA", "NA", "NA", "NA"),
                   uci = c(logit_Sens[3], logit_Spec[3], Sens[3], Spec[3], fpr[3], DOR+qnorm(0.975)*se.DOR , LRp+qnorm(0.975)*se.LRp , 
                           LRn+qnorm(0.975)*se.LRn, "NA", "NA", "NA", "NA", "NA", "NA"),
    ...
    s.matrix[11,1] <- "Log Likelihood Ratio +ve"
    s.matrix[12,1] <- "Log Likelihood Ratio -ve"
    ...
    s.matrix[1,3] <- sprintf('%4.3f', Z_lci[3])
    s.matrix[1,4] <- sprintf('%4.3f', Z_uci[3])
    s.matrix[2,3] <- sprintf('%4.3f', Z_lci[4])
    s.matrix[2,4] <- sprintf('%4.3f', Z_uci[4])
    s.matrix[3,3] <- sprintf('%4.3f', Z_lci[5])
    s.matrix[3,4] <- sprintf('%4.3f', Z_uci[5])
    s.matrix[4,3] <- ""
    s.matrix[4,4] <- ""
    s.matrix[10,3] <- sprintf('%4.3f', Z_lci[6])
    s.matrix[10,4] <- sprintf('%4.3f', Z_uci[6])
    s.matrix[11,3] <- sprintf('%4.3f', Z_lci[7])
    s.matrix[11,4] <- sprintf('%4.3f', Z_uci[7])
    s.matrix[12,3] <- sprintf('%4.3f', Z_lci[8])
    s.matrix[12,4] <- sprintf('%4.3f', Z_uci[8])
    s.matrix[5,3] <- ""
    s.matrix[5,4] <- ""
    s.matrix[6,3] <- ""
    s.matrix[6,4] <- ""
    ...
    s.matrix[9,3] <- ""
    s.matrix[9,4] <- ""
    s.matrix[13,3] <- sprintf('%4.3f', Z_lci[1])
    s.matrix[14,3] <- sprintf('%4.3f', Z_lci[2])
    s.matrix[13,4] <- sprintf('%4.3f', Z_uci[1])
    s.matrix[14,4] <- sprintf('%4.3f', Z_uci[2])
    s.matrix[15, 1:4] <- ""
  ...
  output$revman <- renderTable({
    ...
    s.matrix[3,2] <- sprintf('%4.3f', Z[3,])
    s.matrix[4,2] <- sprintf('%4.3f', Z[4,])
    s.matrix[5,2] <- sprintf('%4.3f', Z[8,])
    s.matrix[6,2] <- sprintf('%4.3f', Z[6,])
    s.matrix[7,2] <- sprintf('%4.3f', Z[7,])
    s.matrix[8,2] <- sprintf('%4.3f', Z[5,])
  ...
  output$SA_input <- renderUI({
    ...
    div(id=letters[(times %% length(letters)) + 1],
        checkboxGroupInput(inputId = "HSROCcheck2", label = h4("Options for ROC Curve tab"),
                           choices = list("Data points"=1, "SROC curve"=2),
                           selected=list(1,2)),
        uiOutput("extrap2"),
  ...
  output$extrap2 <- renderUI({
    if ('2' %in% input$HSROCcheck2){
      checkboxInput(inputId = "extrapp2", label = "Extrapolate SROC curve", value = FALSE)
    }
    else{}
  })
  ...
  output$trials <- renderUI({
    ...
    choicesTrials <- setNames(as.numeric(sadf$Author), sadf$Author)
  ...
  # Create a tbale which displays study-level outcomes
  ...
  output$sroc_sa <- renderPlot({
    ...
    # Calculate sens and spec confidence intervals at the study level
    CIdata_P <- madad(study_level_P, correction.control = "any")
    # Add the confidence intervals to the dataset
    study_level_P$Sens_LCI <- CIdata_P$sens$sens.ci[,1]
    study_level_P$Sens_UCI <- CIdata_P$sens$sens.ci[,2]
    study_level_P$FPR_LCI  <- 1 - CIdata_P$spec$spec.ci[,2]
    study_level_P$FPR_UCI  <- 1 - CIdata_P$spec$spec.ci[,1]
  ...
  output$Prev_input <- renderUI({
    ...
    div(id = letters[(times %% length(letters)) + 1],
        ...
        p("The default value of the slider is the mean value of the prevalence of the disease from the studies included in the analysis."))
  })
